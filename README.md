...
Solana watch prefers `logsSubscribe` when `SOLANA_WSS_URL` is set and only `getTransaction`s signatures whose logs contain `Instruction: Create` or `Instruction: CreateV2` (including inner CPIs). Websocket uses `ping_interval=20` and `ping_timeout=20`. If it drops, stderr prints a sanitized exception (no URL/key), polling prints one line, then watch reconnects (max 10); it does not stay on polling after a drop. `--limit` (default 20) is how many program signatures each poll lists. Stderr includes `signatures=N creates=M skipped_rpc=K null=X error=Y version=V ratelimit=R log_create=A inner_create=B decoded=C`. `decoded=1` always runs a scan and prints `case_id chain token score verdict` (scan failures print `scan_error=<type>` only). `decoded=0` prints `disc_hex=` and `account_count=` for that Pump ix (no full tx dump). `log_create` is Create/CreateV2 in logs, `inner_create` is inner-ix creates, `decoded` is extracted mints; `null` is a missing tx, `version` is an unsupported tx version, `ratelimit` is HTTP 429 / 503 / JSON-RPC -32005 after retries. `getTransaction` uses `jsonParsed` and `maxSupportedTransactionVersion: 1`, retries 429/-32005/503 up to 3 times (1s then 2s backoff), and pauses between Create fetches via `SOLANA_TX_DELAY_MS` (default 120). CLI output never prints the RPC URL or API key. Base watch polls Uniswap V3 `PoolCreated` on the factory in `config/chains.yaml`. Both dedupe tokens, write every scan (including `AVOID`), and print one line per token: `case_id chain token score verdict`. They do not open a browser. `--once` runs a single poll pass (or one Create log if WS is up). Robinhood watch stays off until the Pons factory is filled and `ROBINHOOD_ENABLED=1`.
